...
redrawn from scratch: `scripts/make-deck-figures.py` writes 16 SVGs into
`src/decks/figures/`, transparent and in the deck palette.
...
first, then **close the original and draw from that description**. Keep the
...
dropped and should be paraphrased. The Agile poster is a third-party
infographic; the principles themselves are reproducible from
agilemanifesto.org with its notice, as a list.

## Checking the result

```sh
python3 scripts/make-deck-figures.py           # redraw all 16
